# best-buy-best-buy-order-service
...
To run the necessary services, clone the repo, open a terminal, and navigate to the `best-buy-order-service` directory. Then run the following command:

```bash
docker compose up
```

With the services running, open a new terminal and navigate to the `best-buy-order-service` directory. Then run the following commands:
...
```text
> best-buy-order-service@1.0.0 dev
...
Using the [`test-best-buy-order-service.http`](./test-best-buy-order-service.http) file in the root of the repo, you can test the API. However, you will need to use VS Code and have the [REST Client](https://marketplace.visualstudio.com/items?itemName=humao.rest-client) extension installed.
